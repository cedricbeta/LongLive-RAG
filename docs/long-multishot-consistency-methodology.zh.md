...
```
  被诊断视频的采样帧                          授权 prompt
  (每 shot 4 帧,均匀)                 (global.json:不变量/反例)
         │                                       │
         ▼                                       │
 ┌───────────────────────────┐                   │
 │ ① 诊断 pass(author 角色)  │                   │
 ...
 └───────────────────────────┘                   │
         ...
         └──────────►┌────────────────────────────────────────┐
                     ...
                     └────────────────────────────────────────┘
                                  │ 通过审查的添加项
                                  ▼
      global.json ← "Added invariant: ..."    N.json ← "Maintain invariant: ..."
                                  │
                                  ▼
                    prompt_only / both arm 用精修 prompt 渲染
```
...
```
        方法一的产物                              方法二的产物
      精修 prompt 集                          逐场景 KV 注入计划
 (审查过的不变量添加项)                   (scenes → boundary → 锚帧)
            │                                        │
            ▼                                        ▼
 ┌─────────────────────────┐          ┌──────────────────────────────┐
 │  条件文本通道("应然")    │          │  注意力内容通道("实然")       │
 │  每个 shot 的条件文本     │          │  boundary 处注入历史锚帧 KV   │
 │  携带强化后的不变量描述    │          │  RoPE 重编码 + λ=1.0 偏置     │
 │  → 约束语义:该是什么      │          │  → 提供像素级参照:曾是什么    │
 └─────────────────────────┘          └──────────────────────────────┘
            └──────────────────┬──────────────────────┘
                               ▼
                    同一次生成(both arm)
                               │
                               ▼
        检验两通道是否互补:文本管语义约束,KV 管外观记忆
     (Round 18:both 与 kv_only 高度重合 → 当前增益主要来自 KV 通道)
```
